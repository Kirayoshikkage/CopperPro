...
const resources = () => {
  return gulp.src(path.src.resources).pipe(gulp.dest(path.build.resources));
};
...
const testStyles = () => {
  return gulp.src(testPath.styles).pipe(sass()).pipe(gulp.dest(testFolder));
};

const testScripts = () => {
  return gulp
    .src(testPath.scripts)
...
const testMainTasks = gulp.series(
  ...
  testScripts,
  testHtml,
  clean.bind(this, [`${testFolder}/**/`, `!${testFolder}/*.html`])
);
